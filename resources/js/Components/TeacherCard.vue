      ...
      <div class="flex-1 min-w-0 flex flex-col" :class="layout === 'row' ? 'p-5 sm:p-6 sm:pl-6' : 'pt-4 pr-4 pb-4 pl-1'">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between flex-1">
          <div class="min-w-0 flex-1">
            ...
            <!-- Bio: show snippet or explicit empty state -->
            <p
              v-if="bioSnippet"
              class="text-sm text-slate-600 leading-relaxed mb-3 line-clamp-3"
            ...
            <p
              v-else
              class="text-sm text-slate-400 italic mb-3"
              data-testid="teacher-bio-empty"
            >
            ...
            <div v-if="visibleSubjects.length" class="flex flex-wrap gap-1.5 mb-3">
              <span
            ...
            <div class="flex items-center gap-2">
              <div class="flex">
                <svg v-for="i in 5" :key="i" class="w-4 h-4" :class="i <= Math.round(Number(teacher.rating) || 0) ? 'text-amber-400' : 'text-gray-200'" fill="currentColor" viewBox="0 0 20 20">
                  <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                </svg>
              </div>
              <span class="text-sm font-bold text-slate-900" data-testid="rating">{{ teacher.rating ?? 0 }}</span>
              <span class="text-sm text-slate-500" data-testid="total-reviews">({{ teacher.total_reviews ?? 0 }} reviews)</span>
            </div>
          </div>
