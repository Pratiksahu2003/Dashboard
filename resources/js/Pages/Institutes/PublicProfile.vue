...
const reviewableUserId = computed(() => resolveInstituteUserId(data.value) ?? Number(props.id));
...
/**
 * When the URL is already `/institutes/{slug}-{id}` for the same user id, skip `router.replace`
 * (avoids fighting the server URL / Inertia loops).
 */
function shouldSkipCanonicalInstituteReplace(institute) {
  ...
  const m = (window.location.pathname || '').match(/^\/institutes\/[^/]+-(\d+)\/?$/i);
...
async function submitReviewForm() {
  ...
  try {
    if (usePatch) {
      ...
      await submitTeacherReview(reviewableUserId.value, {
        rating: r,
        title: reviewTitle.value.trim(),
        comment: reviewComment.value.trim(),
      });
...
function navigateToInstitute(row) {
  const path = publicInstituteProfilePath(row);
...
watch(
  ...
  (val) => {
    ...
    const path = publicInstituteProfilePath(val);
    if (!path) return;
